...
////////////////////////////////////////
// Реализация методов класса CEllipse

CEllipse::CEllipse(int x, int y, WORD s) : CBasePoint(x, y, s)
{
	m_wSize = s;
}
...
void CEllipse::Show(CDC* pDC)
{
	...
	PrepareDC(pDC);
	// Рисуем эллипс
	pDC->Ellipse(x - s / 2, y - s, x + s /2, y + s);
	pDC->Ellipse(x - s, y - s/2, x + s, y + s/2);
	// Восстанавливаем контекст
	RestoreDC(pDC);
}
...
////////////////////////////////////////
// Реализация методов класса CStar
...
{
	m_wSize = s/10;
	m_PointsArray.Add(CPoint(x,y+20* m_wSize));
	m_PointsArray.Add(CPoint(x + 6 * m_wSize, y + 6 * m_wSize));
	m_PointsArray.Add(CPoint(x + 20 * m_wSize, y));
	m_PointsArray.Add(CPoint(x + 6 * m_wSize, y -6 * m_wSize));
	m_PointsArray.Add(CPoint(x, y-20 * m_wSize));
	m_PointsArray.Add(CPoint(x-6 * m_wSize, y-6 * m_wSize));
	m_PointsArray.Add(CPoint(x-20 * m_wSize, y));
...
void CSpline::AddPoints(int x, int y, WORD s)
{
	m_wSize = s / 10;
	m_SplinePointsArray.Add(CPoint(x, y + 20 * m_wSize));
	m_SplinePointsArray.Add(CPoint(x+2 * m_wSize, y + 20 * m_wSize));
	m_SplinePointsArray.Add(CPoint(x+5 * m_wSize, y+7 * m_wSize));
	m_SplinePointsArray.Add(CPoint(x + 6 * m_wSize, y + 6 * m_wSize));
	m_SplinePointsArray.Add(CPoint(x+7 * m_wSize, y+5 * m_wSize));
	m_SplinePointsArray.Add(CPoint(x+20 * m_wSize, y+2 * m_wSize));
	m_SplinePointsArray.Add(CPoint(x + 20 * m_wSize, y));
	m_SplinePointsArray.Add(CPoint(x+20 * m_wSize, y-2 * m_wSize));
	m_SplinePointsArray.Add(CPoint(x+7 * m_wSize, y-5 * m_wSize));
	m_SplinePointsArray.Add(CPoint(x + 6 * m_wSize, y - 6 * m_wSize));
	m_SplinePointsArray.Add(CPoint(x+5 * m_wSize, y-7 * m_wSize));
...
//void CSpline::SetCenter(int x, int y, WORD s)
//{
//	m_wSize = s/10;
...
void CSpline::Show(CDC* pDC)
{
	...
	int m_wSize = 75;
	pDC->Polygon(m_PointsArray.GetData(), m_PointsArray.GetSize());
	pDC->PolyBezier(m_SplinePointsArray.GetData(), m_SplinePointsArray.GetSize());
	for (int i = 0; i < m_SplinePointsArray.GetSize(); i++)
		pDC->Ellipse(m_SplinePointsArray[i].x - m_wSize, m_SplinePointsArray[i].y - m_wSize, m_SplinePointsArray[i].x + m_wSize, m_SplinePointsArray[i].y + m_wSize);
	// Восстанавливаем контекст
	RestoreDC(pDC);
}
