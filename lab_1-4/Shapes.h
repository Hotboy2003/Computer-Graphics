...
public:
	...
	// Устанавливает режим рисования полигона
	void SetPolygon(BOOL p) { m_bPolygon = p; };
	//Перегруженный метод трансформации
	void Transform(const CPoint &point0, double ang, int a, int b);
	
};

class CSpline : public CPolygon
{
...
public:
	...
	//void SetCenter(int x, int y, WORD s);
	//Метод рисования сплайна на основе полилайна
	void Show(CDC* pDC);
	//Метод добавления виртуальных контрольных точек для отрисовки сплайна
	void AddPoints(int x, int y, WORD s);
};
